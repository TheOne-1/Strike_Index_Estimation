...
class ProcessorSI:
    ...
    def define_cnn_model(self):
        ...
        # for each feature, add 5 * 1 cov kernel
        kernel_size = int(base_size*.25)
        tower_4 = Conv1D(filters=11, kernel_size=kernel_size, kernel_regularizer=kernel_regu)(main_input)
        tower_4 = MaxPool1D(pool_size=base_size-kernel_size+1)(tower_4)

        # for each feature, add 5 * 1 cov kernel
        kernel_size = base_size / base_size
        tower_5 = Conv1D(filters=11, kernel_size=kernel_size, kernel_regularizer=kernel_regu)(main_input)
    ...
    def evaluate_cnn_model(self):
        ...
        if self.split_train:
            my_evaluator.plot_nn_result(self._y_test, y_pred, 'Strike Index')
        else:
            my_evaluator.plot_nn_result_cate_color(self._y_test, y_pred, self.test_trial_id_list, TRIAL_NAMES,
                                                   'Strike Index')

    def save_cnn_model(self, model_name='si_model'):
    ...
    def find_feature(self):
        ...
        train_all_data_list = ProcessorSI.clean_all_data(train_all_data_list, self.sensor_sampling_fre)
        input_list, output_list = train_all_data_list.get_input_output_list()
        x_train, feature_names = self.convert_input_samples(input_list, self.sensor_sampling_fre)
        y_train = ProcessorSI.convert_output(output_list)
        sub_id_list = train_all_data_list.get_sub_id_list()
        trial_id_list = train_all_data_list.get_trial_id_list()
        ProcessorSI.gait_phase_and_correlation(input_list, y_train, channels=range(self.channel_num))
        ProcessorSI.draw_correlation(x_train, y_train, sub_id_list, SUB_NAMES, feature_names)
        ProcessorSI.draw_correlation(x_train, y_train, trial_id_list, TRIAL_NAMES, feature_names)
        plt.show()
    ...
    def gait_phase_and_correlation(input_list, output_array, channels):
        ...
        for i_channel in channels:
            ...
            for i_sample in range(sample_num):
                ...
                channel_data_resampled = ProcessorSI.resample_channel(channel_data, resample_len)

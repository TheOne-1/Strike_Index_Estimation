...
def draw_f7(rmses, conditions, sigifi_sign_fun):
    def format_ticks():
        ax = plt.gca()
...
if __name__ == "__main__":
    ...
    print("When the training and test data were only from standard shoe trials and minimalist shoe trials, the RMSE were {:.1f}% and {:.1f}%"
          .format(np.mean(rmses['_Trad'])*100, np.mean(rmses['_Minim'])*100))

    with warnings.catch_warnings():
        warnings.simplefilter(action='ignore', category=FutureWarning)
        draw_f7(rmses, conditions, draw_sigifi_sign)
    save_fig('f7', 600)
    plt.show()

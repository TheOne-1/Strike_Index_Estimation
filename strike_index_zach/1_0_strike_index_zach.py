...
def main(basename, run_types):
    my_SI_processor = ProcessorSICrossVali(train, IMU_locations, 200, strike_off_from_IMU=1, do_output_norm=False,
    ...
    if "subject_sufficiently" in run_types:
        ...
        for j in range(7,11):
            ...
            for i in range(1,10):
                ...
                print(trial_name)
                avg_correlation = my_SI_processor.prepare_data_cross_vali(pre_samples=12, post_samples=20, test_name= trial_name, trials = trials, train_num = i)
    ...
    if "window_opt" in run_types:
        ...
        for i, (pre_sample,post_sample) in enumerate(samples):
            ...
            avg_correlation = my_SI_processor.prepare_data_cross_vali(pre_samples=pre_sample, post_samples=post_sample, test_name= trial_name, trials = trials)
    ...
    if "four_cond" in run_types:
        ...
        avg_correlation1 = my_SI_processor.prepare_data_cross_vali(
            pre_samples=pre_samples, post_samples=post_samples, test_name=trial_name, trials=trials, train_num=len(train) - 1)
    ...
    if "hp_opt" in run_types:
        ...
        avg_correlation = my_SI_processor.prepare_data_cross_vali(pre_samples=pre_samples, post_samples=post_samples, test_name= trial_name, trials = trials, train_num=len(train) - 1)
...
if __name__ == "__main__":
    run_types = ["four_cond"]  # "regular_run" "four_cond" "cross_test"
